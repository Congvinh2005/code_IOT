...
//         lcd.setCursor(0, 0);
//         lcd.print(lcdLine1.substring(scrollPos, scrollPos + 16));

//         scrollPos++;
//         if (scrollPos > lcdLine1.length() - 16) scrollPos = 0;
...
const int LED_Thanh = 5;
const int BTN_Thanh = 6;
// Thay đổi: QUAT_Thanh đã là 13
const int QUAT_Thanh = 13;
// Thay đổi: QUAT_BTN_Thanh đã là 11
...
bool lastAlarmBtn = HIGH;
bool lastBtnLed = HIGH;
// Bổ sung Debounce cho Quạt
bool lastBtnFan = HIGH; 
bool lastButton = HIGH;
unsigned long lastButtonTime = 0;
unsigned long lastFanButtonTime = 0; // Thêm thời gian cho nút Quạt
const unsigned long debounceDelay = 150;
...
void updateLCD(float nhietDo, float doAm) {
    ...
    // Thêm trạng thái Quạt vào dòng 2 (cố định)
...
// ================= PIR FILTER =================
bool readPIR_Filter() {
    // Thêm Debounce đơn giản cho PIR
    if (digitalRead(PIR_IN_QUANH) == HIGH) {
        delay(120);
        return digitalRead(PIR_IN_QUANH) == HIGH;
    }
...
void setup() {
    ...
    // Cấu hình chân THANH (LED, Nút LED, Quạt, Nút Quạt)
    pinMode(LED_Thanh, OUTPUT);
    pinMode(BTN_Thanh, INPUT_PULLUP);
    pinMode(QUAT_Thanh, OUTPUT); // Quạt (Chân 13)
    pinMode(QUAT_BTN_Thanh, INPUT_PULLUP); // Nút Quạt (Chân 11)
    ...
    // Đặt trạng thái ban đầu cho Quạt (OFF)
...
void loop() {
    ...
    // ---- Cập nhật OUTPUT ----
    digitalWrite(LED_Thanh, ledState ? HIGH : LOW);
    // Cập nhật trạng thái Quạt (Chân 13)
