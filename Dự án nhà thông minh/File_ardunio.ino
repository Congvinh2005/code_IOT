...
const unsigned long ldrReadInterval = 500; 
...
void updateLCD() {
  // Logic giữ nguyên
  lcdTemp = lcdLine1 + " T:" + String(currentTemperature, 1) + (char)223 +"C H:" + String(currentHumidity, 0) + "% ";
...
void setup() {
  ...
  Serial.println("Arduino sẵn sàng !!!"); 
...
void loop() {
  ...
  if (isDoorAutoOpen) {
      if (millis() - lastDistanceTime >= distanceInterval) {
          ...
          if (distance > 0 && distance <= DISTANCE_THRESHOLD) {
            doorOpenTime = millis(); 
            Serial.println("SIEU_AM_BLOCK -> cửa không đóng"); 
          }
      }
  ...
  digitalWrite(LED_Thanh, finalLedState ? LOW : HIGH); // <<< ĐIỀU KHIỂN LED
