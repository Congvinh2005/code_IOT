...
// =============================================
String BOT_TOKEN = "8240245709:AAEu_ciOfNLNTdn1ANamzCzhGSsT0UtrbPI"; // Điền Token của bạn
String CHAT_ID = "5910648332"; // Điền Chat ID của bạn
...
// Biến chống spam Telegram (không gửi quá nhiều tin cùng lúc)
...
void loop() {
  ...
  // --- 2. NHẬN TÍN HIỆU TỪ ARDUINO (CHUYỂN ĐỘNG) ---
  if (Serial2.available()) {
    String msg = Serial2.readStringUntil('\n');
    msg.trim();
    
    // Nếu Arduino gửi "MOTION" và đã qua thời gian chờ (để tránh spam)
    if (msg == "MOTION") {
...
// =============================================
// ===== HÀM GỬI TELEGRAM (MỚI THÊM VÀO) =======
// =============================================
void sendTelegram(String message) {
  if (WiFi.status() == WL_CONNECTED) {
    
    // Tắt Bluetooth tạm thời để dùng WiFi ổn định
    SerialBT.end(); 
    delay(100);
    ...
    http.begin(url);
    // Bỏ qua kiểm tra chứng chỉ SSL (để chạy nhanh hơn và tránh lỗi CA)
